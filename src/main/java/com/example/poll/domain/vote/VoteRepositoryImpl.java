...
public class VoteRepositoryImpl implements VoteRepositoryCustom {
	...
	public List<Vote> findByChoiceIdWithFetchJoin(Long choiceId) {
		return queryFactory.selectFrom(vote).join(vote.choice, choice).fetchJoin().where(vote.choice.id.eq(choiceId))
				.fetch();
	}

	@Override
	public List<Vote> findByPollIdWithFetchJoin(Long pollId) {
		return queryFactory.selectFrom(vote).join(vote.choice, choice).fetchJoin().join(vote.poll, poll).fetchJoin()
				.where(vote.poll.id.eq(pollId)).fetch();
	}

	@Override
	public Long countByChoiceId(Long choiceId) {
		return queryFactory.selectFrom(vote).where(vote.choice.id.eq(choiceId)).fetchCount();
	}

	@Override
	public Long countByPollId(Long pollId) {
		return queryFactory.selectFrom(vote).where(vote.poll.id.eq(pollId)).fetchCount();
	}
	...
	public List<Vote> findAllWithFetchJoin() {
		return queryFactory.selectFrom(vote).join(vote.poll, poll).fetchJoin().join(vote.choice, choice).fetchJoin().fetch();
	}
}
